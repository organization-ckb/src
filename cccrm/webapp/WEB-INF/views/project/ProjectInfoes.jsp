...
<script type="text/javascript"
	src="${path}/resources/js/jquery-1.8.2.min.js"></script>
<script src="../resources/js/xcConfirm.js" type="text/javascript"
					...
					<div id="one" class="column">
						<div id="part">
							<div class="border">
								...
								<c:if test="${project.status==0}">
									<div class="border2">总工作时间（人/天）</div>
								</c:if>
							</div>
							<div id="cents">
								...
								<c:forEach var="pp" items="${Pprojects}" varStatus="index">
									<div class="centes" onmouseover="shows()">
										<div class="sort">
											<img src="${path}/resources/img/userImage.png" />
										</div>
										<div class="sorts">
											<div class="sortes1">
											...
											<c:if test="${pp.programmer.level==3}">
												<c:set var="level" value="资深工程师" />
											</c:if>
									...
									<div class="modal in" id="signup-modal${index.count}">
										...
										<form action="${path}/project/getExtensionTime" method="post"
											class="white-pink">
											<label><span>项目名称 :</span> <input type="hidden"
												name="project.id" value="${project.id}"> <input
												type="hidden" name="status" value="${pp.status}"> <input
												type="hidden" name="programmer.id"
												value="${pp.programmer.id}"> <input
												readonly="readonly" onFocus="this.blur()" type="text"
												name="proName" value="${project.proName }" /> </label> <label>
												<span>程序员 :</span> <input readonly="readonly"
												onFocus="this.blur()" name="programmer.programmerName"
												type="text" value="${pp.programmer.programmerName}" />
											</label> <label> <span>续约天数 :</span> <input
												name="extension_time" placeholder="请输入数字" type="text" />
											</label> <label> <span>&nbsp;</span> <input type="submit"
												class="button" value="确认申请" />
											</label>
										</form>
									</div>
								</c:forEach>
